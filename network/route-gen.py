...
def generate_traffic_flows():
    with open("traffic_flows.txt", "w") as file:
        for route in routes:
            car_flow = random.randint(10, 100)
            bike_flow = random.randint(10, 100) if route.startswith("attidiya_") or route.startswith("dehiwala_") else car_flow
            ...
            if route.startswith("attidiya_") or route.startswith("dehiwala_"):
                if bike_flow <= car_flow:
                    bike_flow = car_flow + random.randint(1, 10)
            else:
                if car_flow <= bike_flow:
                    car_flow = bike_flow + random.randint(1, 10)
            # Write flows to file
            file.write(f'<flow id="{route}_car" type="motorcar" begin="0" route="{route}" end="50000" departSpeed="max" departPos="base" departLane="best" vehsPerHour="{int(car_flow)}"/>\n')
            file.write(f'<flow id="{route}_bike" type="motorbike" begin="0" route="{route}" end="50000" departSpeed="max" departPos="base" departLane="best" vehsPerHour="{int(bike_flow)}"/>\n')
